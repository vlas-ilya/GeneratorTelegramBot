...
from telegram.dao import names_dao
from telegram.generators.nicknames import generator as nicknames


def _generate_man_name(_surname, _name, _patronymic):
    name = []
    if _surname == 1:
        name.append(names_dao.get_random_man_f())
    if _name == 1:
        name.append(names_dao.get_random_man_i())
    if _patronymic == 1:
        name.append(names_dao.get_random_man_o())
    return " ".join(name)
...
def _generate_woman_name(_surname, _name, _patronymic):
    name = []
    if _surname == 1:
        name.append(names_dao.get_random_woman_f())
    if _name == 1:
        name.append(names_dao.get_random_woman_i())
    if _patronymic == 1:
        name.append(names_dao.get_random_woman_o())
    return " ".join(name)
