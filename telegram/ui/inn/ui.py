...
greetings = {
    'inn_type': 'Выберите тип ИНН',
    'inn_start': 'Если необходимо, введите начало ИНН, и генератор его дополнит или исправит',
}
...
def dispatch(bot, chats, chat_id, command):
    ...
    if command.upper() == 'ИНН' or command.upper() == 'INN':
        chat['processor'] = 'inn'
        ...
        command = "ЮЛ" if command != "ЮЛ" and command != "ИП" else command
        chat["type"] = command
